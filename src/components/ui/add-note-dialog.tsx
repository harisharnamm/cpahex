...
export function AddNoteDialog({ isOpen, onClose, onSubmit, loading = false }: AddNoteDialogProps) {
      ...
      <div className="bg-surface-elevated rounded-xl sm:rounded-2xl shadow-2xl max-w-3xl w-full max-h-[95vh] flex flex-col overflow-hidden animate-scale-in">
        ...
        <div className="flex-shrink-0 flex items-center justify-between p-4 sm:p-6 border-b border-border-subtle">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-text-primary">Add Client Note</h2>
              <p className="text-text-tertiary text-sm">Create a new note for this client</p>
            </div>
          </div>
          <button
            onClick={handleClose}
          ...
          <div className="p-4 sm:p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="Enter tags separated by commas (e.g., important, follow-up, tax-planning)"
              ...
              <p className="text-xs text-text-tertiary">
                Tags help organize and search your notes. Separate multiple tags with commas.
              </p>
            </div>
            ...
            {formData.title && formData.content && (
              <div className="bg-gradient-to-r from-primary/10 to-primary/5 rounded-xl p-4 border border-primary/20">
                <h4 className="font-semibold text-sm text-text-primary mb-3">Note Preview</h4>
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-text-secondary line-clamp-3">{formData.content}</p>
                  {formData.tagsInput && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {formData.tagsInput.split(',').map((tag, index) => (
                        <span
                          key={index}
                          className="inline-flex items-center px-2 py-1 rounded-md bg-primary/20 text-gray-900 text-xs font-medium"
                        >
                          {tag.trim()}
                        </span>
                      ))}

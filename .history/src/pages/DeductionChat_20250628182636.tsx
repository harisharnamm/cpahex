...
export function DeductionChat({ onMenuClick }: DeductionChatProps) {
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-surface to-surface-elevated">
      <TopBar title="Deduction Chat" onMenuClick={onMenuClick} />
      
      <div className="flex-1 max-w-content mx-auto w-full flex flex-col">
        {/* Chat Header */}
        <div className="bg-surface-elevated border-b border-border-subtle p-4 sm:p-6">
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl">
              <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-primary" />
            </div>
            <div>
              <h2 className="text-sm sm:text-base font-semibold text-text-primary">AI Tax Assistant</h2>
              <p className="text-xs sm:text-sm text-text-tertiary hidden sm:block">Ask questions about deductions, receipts, and tax strategies</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
          {messages.map((message) => (
            ...
            >
              <div
                className={`max-w-[280px] sm:max-w-xs lg:max-w-md px-4 sm:px-6 py-3 sm:py-4 rounded-2xl shadow-soft ${
                  message.role === 'user'
        ...
        {/* Input Area */}
        <div className="border-t border-border-subtle bg-surface-elevated p-4 sm:p-6">
          <div className="flex space-x-3 sm:space-x-4">
            <div className="flex-1 relative">
              <textarea
                ...
                className="w-full resize-none rounded-xl border border-border-subtle px-3 sm:px-4 py-2 sm:py-3 bg-surface-elevated text-text-primary placeholder-text-tertiary focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/30 transition-all duration-200 text-sm sm:text-base"
                rows={1}
            ...
            <Button
              ...
              className="shrink-0 w-10 h-10 sm:w-auto sm:h-auto p-2 sm:px-4 sm:py-2"
            >
            </Button>
          </div>
          <p className="text-xs text-text-tertiary mt-2 sm:mt-3 flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-4">

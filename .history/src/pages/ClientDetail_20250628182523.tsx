...
export function ClientDetail({ onMenuClick }: ClientDetailProps) {
  const { id } = useParams();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-surface to-surface-elevated">
      ...
      <div className="max-w-content mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        {/* Client Info Card */}
        <div className="bg-surface-elevated rounded-2xl border border-border-subtle p-4 sm:p-6 mb-6 sm:mb-8 shadow-soft">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs sm:text-sm font-medium text-text-tertiary">Contact</p>
                <p className="text-xs sm:text-sm font-semibold text-text-primary truncate">{client.email}</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs sm:text-sm font-medium text-text-tertiary">Documents</p>
                <p className="text-xs sm:text-sm font-semibold text-text-primary">{documents.length} files</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs sm:text-sm font-medium text-text-tertiary">Tax Year</p>
                <p className="text-xs sm:text-sm font-semibold text-text-primary">2024</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <Tab.Group selectedIndex={selectedTab} onChange={setSelectedTab}>
          <Tab.List className="flex space-x-1 rounded-2xl bg-surface-elevated border border-border-subtle p-1 sm:p-2 mb-6 sm:mb-8 shadow-soft">
            {tabs.map((tab) => (
              <Tab
                key={tab}
                className={({ selected }) =>
                  `w-full rounded-xl py-2 sm:py-3 text-xs sm:text-sm font-semibold leading-5 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary/20 ${
                    selected
          ...
          <Tab.Panels>
            <Tab.Panel className="space-y-6">
              ...
              <div className="bg-surface-elevated rounded-2xl border border-border-subtle overflow-hidden shadow-soft">
                <div className="p-4 sm:p-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0 mb-4">
                    <div>
                      <h3 className="text-base sm:text-lg font-semibold text-text-primary">Upload Documents</h3>
                      <p className="text-xs sm:text-sm text-text-tertiary">Drag and drop files or click to browse</p>
                    </div>
                    <Button
              ...
              {/* Search and Filters */}
              <div className="bg-surface-elevated rounded-2xl border border-border-subtle p-4 sm:p-6 shadow-soft">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {loading ? (
                  ...
                  Array.from({ length: 6 }).map((_, i) => (
                    <div key={i} className="bg-surface-elevated rounded-2xl border border-border-subtle p-4 sm:p-6 shadow-soft animate-pulse">
                  ...
                  documents.map((doc) => (
                    <div key={doc.id} className="group bg-surface-elevated rounded-2xl border border-border-subtle p-4 sm:p-6 shadow-soft hover:shadow-medium hover:-translate-y-1 transition-all duration-200">

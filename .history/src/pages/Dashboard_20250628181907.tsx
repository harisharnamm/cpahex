...
export function Dashboard({ onMenuClick }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-surface to-surface-elevated">
          ...
          <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-6 lg:space-x-8">
                <div>
                  <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-text-primary tracking-tight">Dashboard</h1>
                  <p className="text-sm text-text-tertiary font-medium mt-1">Welcome back, John</p>
                </div>
                <div className="flex items-center space-x-3 bg-surface-elevated rounded-xl border border-border-subtle px-3 sm:px-4 py-2 shadow-soft mt-2 sm:mt-0">
                  <Calendar className="w-4 h-4 text-text-secondary" />
                  <select 
                    value={selectedYear}
                    onChange={(e) => setSelectedYear(e.target.value)}
                    className="bg-transparent border-none text-xs sm:text-sm font-medium text-text-primary focus:outline-none"
                  >
            ...
            <div className="w-full sm:w-80 lg:w-96">
              <div className="relative">
                <Search className="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 w-4 h-4 text-text-tertiary" />
                <Input
                  placeholder="Quick search..."
                  className="pl-10 sm:pl-12 bg-surface-elevated shadow-soft border-border-subtle text-sm"
                />
              </div>
            </div>
          </div>
      ...
      <div className="max-w-content mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        {/* Stats Grid */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-8 space-y-6 lg:space-y-8">
            {/* Upcoming Tasks */}
            <div className="bg-surface-elevated rounded-2xl border border-border-subtle p-4 sm:p-6 lg:p-8 shadow-soft animate-fade-in">
              <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 mb-4 sm:mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <h2 className="text-lg sm:text-xl font-semibold text-text-primary">Upcoming Tasks</h2>
                </div>
                <Button variant="ghost" size="sm" className="self-start sm:self-auto">View All</Button>
              </div>
              <div className="space-y-3 sm:space-y-4">
                {upcomingTasks.map((task) => (
                  <div key={task.id} className="group p-3 sm:p-4 bg-surface rounded-xl border border-border-subtle hover:shadow-medium hover:border-border-light transition-all duration-200">
                    <div className="flex flex-col space-y-2 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-text-primary group-hover:text-primary transition-colors duration-200 text-sm sm:text-base">{task.title}</h3>
            ...
            <div className="bg-surface-elevated rounded-2xl border border-border-subtle p-4 sm:p-6 lg:p-8 shadow-soft animate-fade-in">
              <div className="flex items-center space-x-3 mb-4 sm:mb-6">
                ...
                <h2 className="text-lg sm:text-xl font-semibold text-text-primary">Recent Activity</h2>
              </div>
              <div className="space-y-3 sm:space-y-4">
                {timelineEvents.map((event) => (
                  <div key={event.id} className="flex items-start space-x-3 sm:space-x-4 p-3 rounded-xl hover:bg-surface transition-colors duration-200">
                    <div className="text-base sm:text-lg flex-shrink-0">{getEventIcon(event.type)}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs sm:text-sm font-medium text-text-primary leading-relaxed">{event.event}</p>
          ...
          <div className="lg:col-span-4 space-y-4 sm:space-y-6">
            {/* Quick Actions */}
            <div className="bg-surface-elevated rounded-2xl border border-border-subtle p-4 sm:p-6 shadow-soft animate-fade-in">
              <h3 className="font-semibold text-text-primary mb-3 sm:mb-4 text-sm sm:text-base">Quick Actions</h3>
              <div className="space-y-2 sm:space-y-3">
                <Button className="w-full justify-start text-sm" icon={Plus} size="sm">Add New Client</Button>
                <Button variant="secondary" className="w-full justify-start text-sm" icon={FileText} size="sm">Send W-9 Request</Button>
                <Button variant="secondary" className="w-full justify-start text-sm" icon={AlertTriangle} size="sm">Upload IRS Notice</Button>
              </div>
            </div>

            {/* AI Insights */}
            <div className="bg-gradient-to-br from-primary/5 to-primary/10 rounded-2xl border border-primary/20 p-4 sm:p-6 shadow-soft animate-fade-in">
              <div className="flex items-center space-x-2 mb-3 sm:mb-4">
                <div className="w-2 h-2 bg-primary rounded-full animate-pulse"></div>
                <h3 className="font-semibold text-text-primary text-sm sm:text-base">AI Insights</h3>
              </div>
              <div className="space-y-2 sm:space-y-3">
                {recentInsights.length > 0 ? (
                  recentInsights.map((insight) => (
                    <div key={insight.id} className="p-3 bg-surface-elevated/80 rounded-xl border border-border-subtle">
                      <p className="text-xs sm:text-sm text-text-primary font-medium">{insight.title}</p>
                  ...
                  <div className="p-3 bg-surface-elevated/80 rounded-xl border border-border-subtle">
                    <p className="text-xs sm:text-sm text-text-secondary">No recent AI insights</p>

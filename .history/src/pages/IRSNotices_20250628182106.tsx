...
interface IRSNoticesProps {
  onMenuClick?: () => void;
}

export function IRSNotices({ onMenuClick }: IRSNoticesProps) {
  ...
  if (noticesLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-surface to-surface-elevated">
        <TopBar title="IRS Notices" onMenuClick={onMenuClick} />
        ...
        {!selectedNoticeId ? (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
              <div className="bg-surface-elevated rounded-xl border border-border-subtle p-4 sm:p-6 shadow-soft">
                <div className="flex items-center space-x-3">
                  <div className="p-2 bg-gradient-to-br from-red-100 to-red-50 rounded-xl">
                    <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-red-600" />
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm font-medium text-text-tertiary">Total Notices</p>
                    <p className="text-xl sm:text-2xl font-semibold text-text-primary">{enrichedNotices.length}</p>
                  </div>
                </div>
              </div>
              <div className="bg-surface-elevated rounded-xl border border-border-subtle p-4 sm:p-6 shadow-soft">
